import { API_KEY_FORKIFY, API_URL, RES_PER_PAGE } from './config';
// import { geAJAX } from './helpers';
...
export const loadRecipe = async function (id) {
  try {
    // console.log(data);
    const data = await AJAX(`${API_URL}${id}?key=${API_KEY_FORKIFY}`);
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const data = await AJAX(
      `${API_URL}?search=${query}&?key=${API_KEY_FORKIFY}`
    );
